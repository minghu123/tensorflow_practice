...
class PNN(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            乘以了 F*K,然后reduce_sum 得到了游N*1 ,这样循环了 init_deep_size 后,就得到了, N* init_deep_size 维的输入了; 
            """
            linear_output = []
            ...
            """
             內积要实现的目的为 Ｎ＊Ｆ＊Ｋ　，对这些特征实现两两內积，　Ｎ＊（Ｆ２）特征，　这些特征对没个特征乘以一个权重，然后reduce_sum,根据不同的权重产生 50层 ,就是深度网络的输入层;
             
             內积的方式: 
             embeding 后的N * F *K　　在Ｆ的维度上乘以一个变量，　然后reduce_sum 的基础上上求范数 N*K,然后在N*K 的基础上求范数, 使用范数来作为神经层的输入;
             外积的方式:
             先对embeding在F维上求和,生成一个N*K  然后再生成N*k*k 的tensor ,这个tensor 在K*k 的维度上 乘以K*k的矩阵,最后求和, 得出N*1的tensor,
             经过init_deep_size上,那就生成成了N* init_deep_size 的神经输入! 
            """
            quadratic_output = []
            if self.use_inner:
                for i in range(self.deep_init_size):
